...
class OrderGoodsListView: UIView {
    ...
    override func layoutSubviews() {
        ...
        costLabel.frame = CGRect(x: leftMargin, y: 0, width: frame.width - leftMargin, height: 30)
        lineView1.frame = CGRect(x: leftMargin, y: costLabel.frame.maxY, width: frame.width - leftMargin, height: 1)
        goodsListView.frame = CGRect(x: 0, y: costLabel.frame.maxY, width: frame.width, height: orderGoodsViewHeight)
        feeListView.frame = CGRect(x: 0, y: goodsListView.frame.maxY, width: frame.width, height: feeListViewHeight)
        payMoneyLabel.frame = CGRect(x: frame.width - 100 - 10, y: feeListView.frame.maxY, width: 100, height: 40)
        payLabel.frame = CGRect(x: payMoneyLabel.frame.minX - 60, y: feeListView.frame.maxY, width: 60, height: 40)
        backView.frame = CGRect(x: 0, y: feeListView.frame.maxY, width: frame.width, height: 40)
...
class GoodsView: UIView {
    ...
    override func layoutSubviews() {
        ...
        if isShowImageView {
            giftImageView.frame = CGRect(x: 10, y: (frame.height - 15) * 0.5, width: 40, height: 15)
            titleLabel.frame = CGRect(x: giftImageView.frame.maxX + 5, y: 0, width: frame.width * 0.6, height: frame.height)
            numberLabel.frame = CGRect(x: titleLabel.frame.maxX + 10 - 45, y: 0, width: 30, height: frame.height)
        } else {
            titleLabel.frame = CGRect(x: 10, y: 0, width: frame.width * 0.6, height: frame.height)
            numberLabel.frame = CGRect(x: titleLabel.frame.maxX + 10, y: 0, width: 30, height: frame.height)
        }
...
class FeeListView: UIView {
    ...
    var fee_list: [OrderFeeList]? {
        didSet {
            if fee_list != nil && fee_list!.count > 1 {
    ...
    override func layoutSubviews() {
        ...
        lineView1.frame = CGRect(x: 10, y: 0, width: frame.width - 10, height: 1)
        lineView2.frame = CGRect(x: 10, y: feeListViewHeight - 1, width: frame.width - 10, height: 1)
    }
}
...
class FeeView: UIView {
    ...
    override func layoutSubviews() {
        ...
        titleLabel.frame = CGRect(x: 10, y: 0, width: frame.width - 100, height: 25)
        prictLabel.frame = CGRect(x: frame.width - 150, y: 0, width: 140, height: 25)
    }
}
